<h3><a href="index.php?com=tinloai2_1a&act=add">Thêm tin danh muc</a></h3>
...
<table class="blue_table">
	<tr>
		...
		<th style="width:20%;">Tên tuyển dụng </th> 
	...
	<tr>
		<td style="width:5%;"> <a href="index.php?com=tinloai2_1a&act=edit&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id=<?=$items[$i]['id']?>&curPage=<?=$_REQUEST['curPage']?>" style="text-decoration:none;"><?=$items[$i]['stt']?></a></td>		      
     ...
     <a href="index.php?com=tinloai2_1a&act=edit&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id=<?=$items[$i]['id']?><?php if($_REQUEST['curPage']!='') echo'&curPage='. $_REQUEST['curPage'];?>" style="text-decoration:none;">
	  <img src="../upload/tinloai2_1/<?=$items[$i]['thumb']?>" width="200"/>
      </a>      
		...
		<td style="width:5%;"><a href="index.php?com=tinloai2_1a&act=delete<?php if($_REQUEST['id_cat']!='') echo "&id_cat=".$_REQUEST['id_cat'];?><?php if($_REQUEST['curPage']!='') echo "&curPage=".$_REQUEST['curPage'];?>&id=<?=$items[$i]['id']?>" onClick="if(!confirm('Xác nhận xóa')) return false;"><img src="media/images/delete.png" border="0" /></a></td>
	</tr>
	<?php	}?>
</table>
</form>
<a href="index.php?com=tinloai2_1a&act=add"><img src="media/images/add.jpg" border="0"  /></a>
